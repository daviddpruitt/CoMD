...
/*
 * Pulls in several versions of serial interpolate
 * #defining interpolate specifies which interpolate
 * function to use. Note: only the original interpolate
 * will return correct data
 *
 * Current list of interpolate functions:
 * interpolateOrig         Original implementation
 * interpolateEmpty        Returns values immediately
 * interpolateFakeTable    Uses a (small) local array to store table
 * interpolateLocalTable   Uses a series of values to store
 * interpolateAsmLoad      Only loads the necessary data, does no math
 */

#ifndef interpolate
# define interpolate interpolateOrig
#endif
#include "interpolate.h"
/*
 * This section contains our modifications to eamForce
 *
 * Various versions are compiled using the following defines
 *
 * EAM_FORCE_TYPE           Which EAM force to use
 * SKIP_FORCE_COMPUTATIONS  Loop over boxes but don't do force computations
 * DO_FRIVOLOUS_TIMING      Adds timing for inner loops
 *
 * These only have an effect when EAM_FORCE_TYPE is eamForceAllOpts
 *
 * NEIGHBOR_LIST            Enable neighborlists
 * SEP_LOOPS                Seperate the code into smaller loops
 *                          Compiler does a better job at fusion/fission
 *                          With this option
 * VECTOR_INTERPOLATE       Enables vectorized interpolation. Note: this
 *                          only works with neighborlists
 */

 #ifndef EAM_FORCE_TYPE
 # define EAM_FORCE_TYPE eamForceAllOpts
 #endif
...
// prefetch the data in the specified interpolation table
...
// Prefetch the data in the box representing
// the physical space
PREFETCH_TYPE prefetchBox(int box, SimFlat *s)
{
  ...
  // We don't need to fetch every single piece of data
  // cache lines are typically 64 bytes wide so only fetch
  // every 64th piece of data
  int stride = 64/(sizeof(PREFETCH_TYPE)*3);

  // loop over atoms in iBox
  for (int iOff=MAXATOMS*box,ii=0; ii<nBox; ii+=stride,iOff+=stride)
  {
    /* we can fetch data across x, y, z dimensions or just one
       Doesn't seem to make a difference, probably due to the same
       reasons as above, all dimensions are stored next to each other */
/*    for (int k=0; k<3; k++)*/
...
// prefetch the i and j box, different since
// an atom in one box can interact with the atoms from a
// neighboring box and within eamforce i != j
...
/* iterate over boxes without doing force compuations
   allows one to measure how much time force computations add
   compiler will optimize out if that always evaluates false

   Also includes code to time inner loops (frivStartTimer)
*/
...
void interpolateOLD(InterpolationObject* table, real_t r, real_t* f, real_t* df)
{
